...
func New(logger *slog.Logger, config config.HttpAdapter, chatUC *usecase_chat.UseCase) *HttpAdapter {
	router := newRouter(logger, chatUC)
...
func newRouter(logger *slog.Logger, chatUC *usecase_chat.UseCase) http.Handler {
	r := router.New()

	ctr := controller.New(logger, chatUC)
